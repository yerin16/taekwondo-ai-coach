...
extension VideoProcessingChain {
    ...
    private func checkpose(_ actionPrediction: ActionPrediction) {
        ...
        if actionPrediction.label == "NoAction" && actionPrediction.confidence > 0.85 && isStanceDetected == false {
            print("Stance detected")
            isStanceDetected = true
            
            DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                isStanceDetected = false
            }
//
//            DispatchQueue.main.async {
