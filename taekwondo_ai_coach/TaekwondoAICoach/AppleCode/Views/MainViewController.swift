...
extension MainViewController {
    ...
    private func checkpose(_ actionPrediction: ActionPrediction) {
        if actionPrediction.label == "CorrectStance" && actionPrediction.confidence >= 0.80 {
        ...
        } else if actionPrediction.label == "CorrectStance" && actionPrediction.confidence > 0.50 && actionPrediction.confidence < 0.80 {
            DispatchQueue.main.async {
                self.checkLabel.text = "Try Harder!"
            }
        }
    }
